...
internal fun requireBenchmarkTargetPackage(): String {
    val configured = InstrumentationRegistry.getArguments()
        .getString("benchmark.targetPackage")
        .orEmpty()
        .trim()
    require(configured.isNotBlank()) {
        "Macrobenchmark requires -e benchmark.targetPackage <profileable target package>, for example com.android.purebilibili.dev."
    }
    return configured
}
