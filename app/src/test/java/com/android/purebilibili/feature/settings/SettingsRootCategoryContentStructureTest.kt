...
class SettingsRootCategoryContentStructureTest {
    ...
    fun settingsSubpages_useNagramStyleSectionNames() {
        ...
        // P3 外观页五组信息架构（替代旧的"显示模式/字体与密度"单组）
        assertTrue(appearance.contains("AppPreferenceSectionTitle(\"界面与明暗\")"))
        assertTrue(appearance.contains("AppPreferenceSectionTitle(\"颜色\")"))
        assertTrue(appearance.contains("AppPreferenceSectionTitle(\"表面与效果\")"))
        assertTrue(appearance.contains("AppPreferenceSectionTitle(\"文字与显示\")"))
        assertTrue(appearance.contains("AppPreferenceSectionTitle(\"高级外观覆盖\")"))
