...
class LongPressSpeedHintSettingsPolicyTest {
    ...
    fun `hint scale clamps to supported range`() {
        assertEquals(LONG_PRESS_SPEED_HINT_SCALE_MIN, normalizeLongPressSpeedHintScale(0.45f))
        assertEquals(0.5f, normalizeLongPressSpeedHintScale(0.5f))
    ...
    fun `hint alpha clamps to supported range`() {
        assertEquals(LONG_PRESS_SPEED_HINT_ALPHA_MIN, normalizeLongPressSpeedHintAlpha(0.05f))
        assertEquals(0.1f, normalizeLongPressSpeedHintAlpha(0.1f))
